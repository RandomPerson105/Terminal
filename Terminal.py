...
from CoffeeShop import menu1, menu2, price
from phonenumbers import carrier
import phonenumbers
from TerminalRoot import main
...
header = print("Terminal [BETA]")
print("Type 'help' for more information.")


while True:
    path = os.getcwd()
    code = input("┌──(" + host_name + "@Terminal)" +
                 "~" + "[" + path + "]" + "\n" + "└─$ ")
    if code == 'ping':
    ...
    if code == 'nano':
        ...
        if platform.system() == 'Linux':
            os.system("nano")
    if code == 'root':
        print("Warning u cannot exit root unless you reopen the terminal")
    ...
    if code == 'terminal -barista':
        print("\nWelcome to terminal's coffee shop\n")
        name = input("Enter your name: ")
        print("Hello " + name + " Here are some of our products")
        print("Menu bar:")
        print(menu1)
        print(menu2)

        product = input("Enter the number of the product you want: ")

        price = price

        quantity = input("Enter the quantity of the product: ")

        total_dol = price * int(quantity)
        total_php = price * 50.30 * int(quantity)
        print("Hey " + name + " Total: $" + str(total_dol))
        print("Hey " + name + " Total: ₱" + str(total_php))
    if code == 'crypto -wW':
        currency = "USD"
        metric = "Close"
